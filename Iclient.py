...
def Main(username, address, port, changed):
    ...
    class MainP:
        ...
        def __init__(self, master):
            ...
            self.ilabelserver = Label(self.icanvas, text='Server Address', bg='black', fg='white',
            ...
            def get_info():
                if len(self.ientryuser.get()) > 0 and len(self.ientryserver.get()) > 0:
                    ...
                    store_user, store_address, store_port = self.ientryuser.get(), self.ientryserver.get(), int(
                        self.ientryport.get())
            ...
            self.username = Label(self.bottom_frame, width=10, text='username', background='white', fg='black',
                                  font=('Arial', 14))
    ...
    def exit_program():
        if messagebox.askokcancel("Leaving chat", "Are you sure mate?"):
            global send_final
            shutdown = True
            root.destroy()
